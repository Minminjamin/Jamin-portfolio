        ...
        <div className=' bg-indigo-100 flex justify-center flex-col'>
            ...
            <div className='flex flex-col items-center'>
                {projectData.map(item => (
                    <div key={item.id} className = 'my-5 w-1/2 bg-white rounded-lg flex items-center shadow-xl'>
                        ...
                        <div className='flex-col flex'>
                            ...
                            <h3 className='font-bold'>프로젝트에 사용한 스택들</h3>
                            <span className='mb-3'>{item.skills}</span>

                            <span><span className='font-bold mr-3'>외부 링크</span><a href={item.gitHub} className="text-black hover:text-blue-700">깃허브 바로가기</a></span>
                        </div>
                    </div>
                ))}
            </div>
